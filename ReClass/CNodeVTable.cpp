...
void CNodeVTable::Update(CHotSpot & Spot)
{
	StandardUpdate(Spot);
}
...
int CNodeVTable::Draw(ViewInfo & View, int x, int y)
{
	...
	x = AddText(View, x, y, crName, HS_NAME, _T("%s"), m_strName) + g_FontWidth;
	//else
	//	x = AddText(View, x, y, crName, HS_NONE, _T("%s_vtable"), pParent->m_strName) + FontWidth;
